...
RSpec.describe UsersController, type: :controller do
  ...
  describe 'responds to' do
    it 'create' do
      ...
      expect(response.status).to include 'User was successfully created.'
    end
    ...
    it 'delete' do
      ...
      expect(response.media_type).to eq 'text/html'
      expect(response.status).to eq 200
      expect(response.status).to include 'User was successfully destroyed.'
    end
    ...
    it 'update' do
      ...
      expect(response.media_type).to eq 'text/html'
      expect(response.status).to eq 200
      expect(response.status).to include 'User was successfully updated.'
    end
